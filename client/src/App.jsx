...
const journeySteps = [
	...
	},
	{
		...
		copy: "Build reusable Panchakarma templates that map Snehan, Svedana, and Shodana combinations across the cleanse duration.",
	},
	{
			...
			<header className="relative overflow-hidden">
				...
				<div className="section-container flex items-center justify-between py-6">
					...
					<nav className="hidden items-center gap-8 text-sm font-medium text-slate-300 lg:flex">
						<a href="#features" className="hover:text-emerald-200">Why VaidyaDesk</a>
						<a href="#therapies" className="hover:text-emerald-200">Therapies</a>
						<a href="#journey" className="hover:text-emerald-200">Journey</a>
						<a href="#testimonials" className="hover:text-emerald-200">Voices</a>
					</nav>
					<div className="flex items-center gap-3">
						<button className="rounded-full border border-emerald-400/40 px-4 py-2 text-sm font-medium text-emerald-200 transition hover:border-emerald-300/60 hover:text-emerald-100">
							Explore Demo
						</button>
						<button className="hidden rounded-full bg-emerald-400 px-4 py-2 text-sm font-semibold text-slate-950 shadow-lg shadow-emerald-500/40 transition hover:bg-emerald-300 lg:inline-flex">
							Start Free
						</button>
					</div>
				</div>
				...
				<div className="section-container grid gap-10 pb-24 pt-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
					<div className="space-y-8">
						...
						<div className="flex flex-wrap gap-4">
							<button className="flex items-center gap-2 rounded-full bg-emerald-400 px-6 py-3 text-sm font-semibold text-slate-950 shadow-lg shadow-emerald-500/40 transition hover:bg-emerald-300">
								Build a Panchakarma Plan
							</button>
							<button className="flex items-center gap-2 rounded-full border border-white/15 px-6 py-3 text-sm font-semibold text-white transition hover:border-white/40">
							...
							<div className="mt-6 space-y-4">
								{[
									...
									>
										...
										<p className="mt-1 text-xs text-emerald-200">{item.subtitle}</p>
				...
				<section id="therapies" className="bg-slate-900/40 py-16">
					...
					<div className="section-container mt-12 grid gap-6 lg:grid-cols-2">
						{therapies.map((therapy) => (
							...
							>
								<h3 className="text-xl font-semibold text-white">{therapy.name}</h3>
								<p className="mt-3 text-sm leading-relaxed text-slate-300">{therapy.description}</p>
							</article>
						))}
					</div>
				</section>
					...
					<div className="mt-12 grid gap-6 md:grid-cols-2">
						{journeySteps.map((step, index) => (
							...
							>
								...
								<p className="mt-4 text-sm leading-relaxed text-slate-300">{step.copy}</p>
							</div>
						))}
					</div>
				</section>
						...
						<div>
							...
							<ul className="mt-6 space-y-3 text-sm text-slate-200">
								<li className="flex items-start gap-3">
									...
									<span>Dynamic scheduling that adapts to therapist availability and room resources in real time.</span>
								</li>
								<li className="flex items-start gap-3">
									<span className="mt-1 text-emerald-300">✓</span>
									<span>Pre- and post-therapy checklists that honour dosha-specific diet, rest, and herbal support.</span>
								</li>
								<li className="flex items-start gap-3">
									<span className="mt-1 text-emerald-300">✓</span>
									<span>Rich feedback loop to capture symptom shifts, emotional wellbeing, and energy levels.</span>
								</li>
							</ul>
						</div>
						<div className="rounded-2xl border border-emerald-400/20 bg-emerald-500/10 p-6 text-slate-100">
							...
							<div className="mt-5 space-y-4 text-sm">
								<div className="rounded-2xl border border-white/20 bg-white/10 p-4">
									<p className="font-semibold text-white">12 hrs before Virechan</p>
									<p className="mt-1 text-xs text-emerald-200">SMS · Hydration, light meals, rest guidance</p>
								</div>
								<div className="rounded-2xl border border-white/15 bg-white/5 p-4">
									<p className="font-semibold text-white">Post Nasya check-in</p>
									<p className="mt-1 text-xs text-emerald-200">In-app · Breathing exercises, head massage tips</p>
								</div>
								<div className="rounded-2xl border border-white/10 bg-white/5 p-4">
									<p className="font-semibold text-white">Day 10 follow-up</p>
									<p className="mt-1 text-xs text-emerald-200">Email · Lifestyle sheet, herbal support update</p>
								</div>
							</div>
			...
			<footer className="border-t border-white/10 bg-slate-950/80 py-10">
				<div className="section-container flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between text-sm text-slate-400">
					<div>
						...
						<p className="text-xs uppercase tracking-[0.35em] text-slate-500">Panchakarma Operating System</p>
					</div>
					<div className="flex flex-wrap gap-4">
						<a href="#features" className="hover:text-emerald-200">Features</a>
						<a href="#therapies" className="hover:text-emerald-200">Therapies</a>
						<a href="#journey" className="hover:text-emerald-200">Journey</a>
						<a href="#testimonials" className="hover:text-emerald-200">Testimonials</a>
					</div>
					<p className="text-xs text-slate-600">© {new Date().getFullYear()} VaidyaDesk. All rights reserved.</p>
				</div>
			</footer>
		</div>
	);
